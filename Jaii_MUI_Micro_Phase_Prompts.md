...
## Phase 6 — Light and dark theme generation

**Model:** `deepseek/deepseek-v4-flash`  
**Cost:** Very low  
**Outcome:** Complete light/dark/system behavior

```text
Implement light, dark, and system modes using the Phase 5 theme architecture.

Requirements:
- complete semantic tokens in both modes
- system mode follows OS preference changes
- selected mode remains distinct from resolved mode
- no page reload
- a temporary small mode selector on the proof page
- accessible surfaces, borders, text, menus, dialogs, and inputs

Do not add color presets or the final customizer drawer.
```

**Gate:** Light, dark, and system visibly affect all proof components, including open Dialog and menu surfaces.

---

## Phase 7 — Six primary-color presets

**Model:** `deepseek/deepseek-v4-flash`  
**Cost:** Very low  
**Outcome:** Minimal-inspired tonal presets

```text
Add six generated primary presets to the theme:
- Emerald #00A76F
- Cyan #078DEE
- Purple #7635DC
- Blue #0C68E9
...
Each preset must generate lighter, light, main, dark, darker, contrast, hover, selected, focus, and translucent states. Add a temporary preset selector to the proof page. Keep Cyan as default. Remove hard-coded primary colors from proof components.
```

**Gate:** Every preset is attractive and readable in both light and dark modes.

---

## Phase 8 — Shape, density, contrast, and typography preferences

**Model:** `deepseek/deepseek-v4-flash`  
**Cost:** Very low  
**Outcome:** Remaining theme dimensions work

```text
Add working theme preferences for:
...
Expose temporary controls on the proof page. Apply the settings to real MUI cards, inputs, buttons, tables, dialogs, and navigation-sized samples.
```

**Gate:** Each setting causes an obvious global change, not only a label or stored value.

---

## Phase 9 — Preference store and first-paint bootstrap

**Model:** `z-ai/glm-5`  
**Cost:** Controlled premium spend  
**Outcome:** Persistent customization without flash

```text
Install and use Zustand for a versioned appearance preference store.
...
Implement safe parsing, defaults, migration shape, reset, and pre-hydration bootstrap so mode and direction do not flash. Keep language preference compatible with i18next. Do not build the final drawer.
```

**Gate:** Refresh preserves all current controls and there is no obvious light/direction flash.
...
Use original Jaii styling. Connect the reset button, but leave detailed controls to the next two phases.
```
...
```text
Connect these existing preferences to polished controls inside the customizer:
- light/dark/system
...
Use visual thumbnails and swatches inspired by the supplied references but with original Jaii artwork. Connect every option to the preference store.
```
...
- grouped results
- recent routes in local preference state
- keyboard navigation
...
```text
Create a polished notification popover with local fictional data:
- unread badge
...
```text
Implement a simple demo-session provider with three switchable roles:
- owner
- manager
- staff

Add protected dashboard routing, demo login redirect, user menu role switch for development, and a small permission helper. Do not implement real tokens, refresh, or backend security.
```
